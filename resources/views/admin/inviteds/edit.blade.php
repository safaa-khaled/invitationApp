                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('inviteds.update', [$invited->id]) }}">
                        ...
                        <div class="form-group row">
                            <div class="col-md-6">
                                <label for="title1" class="col-md-3 col-form-label text-md-right"> <h5>اللقب الأول</h5></label>
                                <br/>
                                    <input type="radio" id="maali" name="title1" value="معالي" @if ($invited->title1=="معالي") checked @endif>
                                     <label for="maali">معالي</label>
                                    <input type="radio" id="saada" name="title1" value="سعادة" @if ($invited->title1=="سعادة") checked @endif>
                                    <label for="saada">سعادة</label>
                                    <input type="radio" id="mr" name="title1" value="mr" @if ($invited->title1=="mr") checked @endif>
                                    <label for="mr">mr</label>
                                    <input type="radio" id="Your Excellency" name="title1" value="Your Excellency" @if ($invited->title1=="Your Excellency") checked @endif>
                                    <label for="Your Excellency">Your Excellency</label>
                            </div>
                            <div class="col-md-6">
                                <label for="title2" class="col-md-3 col-form-label text-md-right"> <h5>اللقب الثاني</h5></label>
                                
                                    <select class="form-control" name="title2" id="title2"
                                    data-select-search="true" required>
                                    <option value="">اختر اللقب</option>
                                    @foreach ($titles as $title)
                                        <option value="{{ $title->id }}" @if ($invited->title2==$title->id) selected @endif>{{ $title->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
